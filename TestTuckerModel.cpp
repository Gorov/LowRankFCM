...
void TestTuckerModel::Init(char* embfile, char* trainfile, FeaParams* params)
{
...
void TestTuckerModel::BuildModelsFromData(char* trainfile) {
    labeldict["Other"] = 0;
...
//    lab_model -> InitEmb(fea_params.fea_dim);
    ...
    LrTensorTuckerModel* pmodel = new LrTensorTuckerModel(fea_model, emb_model, lab_model, fea_params.rank1, fea_params.rank2, fea_params.rank3);
...
int TestTuckerModel::LoadInstanceInit(ifstream &ifs) {
    int id;
    int fea_id;
...
int TestTuckerModel::LoadInstance(ifstream &ifs, int type) {
    return LoadInstance(ifs);
}

int TestTuckerModel::LoadInstance(ifstream &ifs) {
    //    int id, model_id;
...
string TestTuckerModel::ProcSenseTag(string input_type) {
    size_t idx = input_type.find_first_of(".");
    string ret = input_type.substr(idx + 1);
    return ret; 
}

string TestTuckerModel::ProcNeTag(string input_type) {
    size_t idx = input_type.find_first_of(":");
    string ret = input_type.substr(idx + 1);
...
string TestTuckerModel::ToLower(string& s) {
...
int TestTuckerModel::AddWordFeature(string feat_key, int pos) {
    int fea_id;
...
void TestTuckerModel::ForwardProp()
{
    real sum;
    int c;
...
void TestTuckerModel::TrainData(string trainfile, string devfile, int type) {
...
void TestTuckerModel::EvalData(string trainfile, int type) {
    int total = 0;
    int right = 0;
    int positive = 0;
    int tp = 0;
...
void TestTuckerModel::PrintInstance(ofstream& ofs, LrFcemInstance* p_inst) {
...
void TestTuckerModel::SetModels() {
...
void TestTuckerModel::PrintModelInfo() {
...
void TestTuckerModel::WeightDecay(real eta_real, real lambda) {
}
