...
void TestFullModel2::Init(char* embfile, char* trainfile)
{
...
void TestFullModel2::BuildModelsFromData(char* trainfile) {
    //    if (type == "SEM_EVAL") {
    //        labeldict["Other"] = 0;
    //    }
    //    else labeldict["NA"] = 0;
...
int TestFullModel2::LoadInstanceInit(ifstream &ifs) {
    int id;
    int fea_id;
...
int TestFullModel2::LoadInstance(ifstream &ifs, int type) {
    return LoadInstance(ifs);
}

int TestFullModel2::LoadInstance(ifstream &ifs) {
    //    int id, model_id;
...
string TestFullModel2::ProcSenseTag(string input_type) {
    size_t idx = input_type.find_first_of(".");
    string ret = input_type.substr(idx + 1);
    return ret; 
}

string TestFullModel2::ProcNeTag(string input_type) {
    size_t idx = input_type.find_first_of(":");
    string ret = input_type.substr(idx + 1);
...
string TestFullModel2::ToLower(string& s) {
...
int TestFullModel2::AddWordFeature(string feat_key, int pos) {
    int fea_id;
...
void TestFullModel2::ForwardProp()
{
    real sum;
    int c;
...
void TestFullModel2::TrainData(string trainfile, string devfile, int type) {
...
void TestFullModel2::EvalData(string trainfile, int type) {
    int total = 0;
    int right = 0;
    int positive = 0;
    int tp = 0;
...
void TestFullModel2::PrintInstance(ofstream& ofs, LrFcemInstance* p_inst) {
...
void TestFullModel2::EvalData(string trainfile, string outfile, int type) {
    int total = 0;
    int right = 0;
    int positive = 0;
    int tp = 0;
...
void TestFullModel2::PrintModelInfo() {
...
void TestFullModel2::WeightDecay(real eta_real, real lambda) {
