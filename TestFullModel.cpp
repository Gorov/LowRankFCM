...
void TestFullModel::Init(char* embfile, char* trainfile)
{
...
void TestFullModel::Init(char* embfile, char* trainfile, FeaParams* params)
{
    fea_params.head = true;
    fea_params.word_on_path = true;
    fea_params.word_on_path_type = true;
...
void TestFullModel::BuildModelsFromData(char* trainfile) {
    //    if (type == "SEM_EVAL") {
    //        labeldict["Other"] = 0;
    //    }
    //    else labeldict["NA"] = 0;
...
int TestFullModel::LoadInstance(ifstream &ifs, int type) {
    return LoadInstance(ifs);
}

int TestFullModel::LoadInstance(ifstream &ifs) {
    //    int id, model_id;
...
string TestFullModel::ProcSenseTag(string input_type) {
    size_t idx = input_type.find_first_of(".");
    string ret = input_type.substr(idx + 1);
    return ret; 
}

string TestFullModel::ProcNeTag(string input_type) {
    size_t idx = input_type.find_first_of(":");
    string ret = input_type.substr(idx + 1);
...
string TestFullModel::ToLower(string& s) {
...
int TestFullModel::AddWordFeature(string feat_key, int pos) {
    int fea_id;
...
void TestFullModel::ForwardProp()
{
    real sum;
    int c;
...
void TestFullModel::TrainData(string trainfile, string devfile, int type) {
    if (lr_fcem_list.size() != 0) {
        printf("L-emb1: %lf\n", lr_fcem_list[0] -> label_emb[0]);
        printf("L-emb2: %lf\n", lr_fcem_list[0] -> label_emb[word_emb_dim]);
        printf("L-emb1: %lf\n", lr_fcem_list[0] -> fea_model -> syn0[0]);
        printf("L-emb2: %lf\n", lr_fcem_list[0] -> fea_model -> syn0[word_emb_dim]);
    }
...
void TestFullModel::EvalData(string trainfile, int type) {
    int total = 0;
    int right = 0;
    int positive = 0;
    int tp = 0;
...
void TestFullModel::PrintInstance(ofstream& ofs, LrFcemInstance* p_inst) {
...
void TestFullModel::EvalData(string trainfile, string outfile, int type) {
    int total = 0;
    int right = 0;
    int positive = 0;
    int tp = 0;
...
void TestFullModel::PrintModelInfo() {
...
void TestFullModel::WeightDecay(real eta_real, real lambda) {
