...
void RunRankingModel::Init(char* embfile, char* trainfile, PPA_Params* params)
{
    fea_params.sum = true;
...
void RunRankingModel::BuildModelsFromData(char* trainfile) {
    ...
    LrTuckerRankingModel* pmodel = new LrTuckerRankingModel(fea_model, emb_model, lab_model, fea_params.rank1, fea_params.rank2, fea_params.rank3);
...
int RunRankingModel::LoadInstanceInit(ifstream &ifs) {
    int fea_id;
    string token, clus;
    ...
    p_inst -> Clear();
    {
        ...
        if (fea_params.wordnet) {
            ifs.getline(line_buf2, 5000, '\n');
            istringstream iss3(line_buf2);
            iss3 >> wordnet_len;
            for (int j = 0; j < wordnet_len; j++) {
                iss3 >> senses[j];
            }
        }
    }
    for (int i = 0; i < p_inst -> list_len; i++)
    {
        ...
        if (fea_params.wordnet) {
            ifs.getline(line_buf2, 5000, '\n');
            istringstream iss3(line_buf2);
            iss3 >> head_wordnet_len;
            for (int j = 0; j < head_wordnet_len; j++) {
                iss3 >> head_senses[j];
            }
        }
        ...
        for (int j = 0; j < 2; j++) {  
            ...
            if (fea_params.clus) {
                ...
                oss.str("");
                oss << "PAIR_CLUS_" << p_inst -> clus[0] << "_" << clus;
                slot_key = oss.str();
                fea_id = fea_model -> AddFeature(slot_key);
            }
            ...
            if (fea_params.nextpos) {
                ostringstream oss;
                oss << "NEXT_POSTAG_" << nextpos << "_POSITION_" << j;
                slot_key = oss.str();
                fea_id = fea_model -> AddFeature(slot_key);
            }
            ...
            if (fea_params.prep) {
                ostringstream oss;
                oss.str("");
                oss << "PREP_" << ((PPAInstance*)inst) -> prep_word << "_POSITION_" << j;
                slot_key = oss.str();
                fea_id = fea_model -> AddFeature(slot_key);
            }
            ...
            if (fea_params.verbnet) {
                ostringstream oss;
                oss << "HEADTAG" << headpos << "_POSITION_" << j;
                slot_key = oss.str();
                fea_id = fea_model -> AddFeature(slot_key);
//                if (j == 1) {
                    int match = 0;
                    for (int k = 0; k < verbnet_len; k++) {
                        ...
                        oss.str("");
                        oss << "HEAD_VERBPREP_" << preps[k] << "_POSITION_" << j;
                        slot_key = oss.str();
                        fea_id = fea_model -> AddFeature(slot_key);
                    }
                    if (match == 1) {
...
int RunRankingModel::LoadInstance(ifstream &ifs) {
...
string RunRankingModel::ProcSenseTag(string input_type) {
    size_t idx = input_type.find_first_of(".");
    string ret = input_type.substr(idx + 1);
    return ret; 
}

string RunRankingModel::ProcNeTag(string input_type) {
    size_t idx = input_type.find_first_of(":");
    string ret = input_type.substr(idx + 1);
...
string RunRankingModel::ToLower(string& s) {
...
int RunRankingModel::AddWordFeature(string feat_key, int pair_id, int pos) {
    int fea_id;
...
void RunRankingModel::ForwardProp()
{
    real sum;
    int c;
...
void RunRankingModel::TrainData(string trainfile, string devfile) {
...
void RunRankingModel::EvalClosest(string trainfile) {
    int total = 0;
    int right = 0;
    real acc = 0.0;
    ifstream ifs(trainfile.c_str());
...
void RunRankingModel::EvalData(string trainfile) {
    int total = 0;
    int right = 0;
    real acc = 0.0;
    double max, max_p;
...
void RunRankingModel::SetModels() {
...
void RunRankingModel::PrintModelInfo() {
